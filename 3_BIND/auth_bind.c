...
int add(sqlite3* db, char* email, char* password) {
  ...
  char* req = "INSERT INTO users (id, email, password) VALUES(?, ?, ?)";
  char buffer[20];
  update_usr_id(db);
  ...
  printf("%s\n", req);
  sqlite3_stmt* stmt;
  sqlite3_prepare(db, req, strlen(req), &stmt, &req);
  ...
  int execDesk;
  // while (1) {
    execDesk = sqlite3_step(stmt);
    // if (execDesk == SQLITE_DONE)
    //   break;
    // if (execDesk == SQLITE_ERROR) {
    //   printf("%s\n", "error add ");
    //   sqlite3_free(execDesk);
    // }
  // }//execDesk = sqlite3_step(stmt);
  // sqlite3_finalize(stmt);
